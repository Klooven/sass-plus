...
async function process(style, config) {
  ...
  if (clean !== false) {
    try {
      const {content, safelist, fontFace, keyframes, variables} = clean;
      ([{css: currentCSS}] = await new PurgeCSS().purge({css: [{raw: currentCSS}], content, safelist, fontFace, keyframes, variables}));
    } catch {
