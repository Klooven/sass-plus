...
async function process(style, config) {
  ...
  try {
    ({css: currentCSS} = sass.renderSync({data: style, includePaths: ['node_modules', ..._sassInclude]}));
  } catch {
  ...
  if (clean !== false) {
    try {
      ([{css: currentCSS}] = await new PurgeCSS().purge({css: [{raw: currentCSS}], content: clean}));
    } catch {
